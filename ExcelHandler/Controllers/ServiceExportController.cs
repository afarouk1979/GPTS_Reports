...
public class ServiceExportController : ControllerBase
{
    ...
    /// <summary>
    /// A helper class to convert numeric values into their Arabic word representation.
    /// Handles integers and fractions (Qirsh).
    /// </summary>
    public static class TafkeetHelper
    {
        /// <summary>
        /// Converts a decimal number to its full Arabic word representation.
        /// </summary>
        /// <param name="number">The decimal number to convert.</param>
        /// <returns>The Arabic text representation of the number.</returns>
        public static string ToWords(decimal number)
        {
            ...
            // Handle the fractional part (Qirsh).
            if (fractionPart > 0)
            {
                // Multiply by 100 and round to get the qirsh value as an integer.
                long fractionalValue = (long)Math.Round(fractionPart * 100);
                if (fractionalValue > 0)
                {
                    if (!string.IsNullOrEmpty(words))
                        words += " و "; // Add "and" if there is an integer part.
        ...
        private static string ConvertIntegerToWords(long number)
        {
            ...
            string words = "";
            bool needsWaw = false; // To handle adding "و" correctly.
            ...
            [FromQuery] DateTime? startDate = null, [FromQuery] DateTime? endDate = null)
    {
        try
        {
            ...
            else
            {
                ...
                using var emptyMemoryStream = new MemoryStream(); // تغيير اسم المتغير هنا
            ...
            // Sanitize column names by removing newlines.
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                // Calculate total for the row first.
                double rowTotal = 0;
                if (!isCancelled)
                {
                ...
                foreach (var service in serviceColumns)
                {
                    ...
                    {
                        reportSheet.Cell(currentRow, service.Value).Value = isCancelled ? 0 : value;
                    }
                    else
                    {
            ...
            for (int i = 9; i <= totalColIndex; i++)
            {
                ...
                reportSheet.Cell(summaryRow, i).FormulaA1 = $"SUM({colLetter}{headerRow + 1}:{colLetter}{currentRow - 1})";
            ...
            using var reportMemoryStream = new MemoryStream(); // تغيير اسم المتغير هنا
